...
fname = sprintf('simulations/circle/circle_t%d_s%.2f_trial%d.mat',tau, sigma, rep);
...
save(sprintf('out/circle/Dist_s%.2f.mat',sigma),...
     'algos','Mouts','proj_Mouts','infos', 'ts', 'Dist2', 'Dist2_move', 'avgdists','maxdists');
 

%% boxplot

MatName = sprintf('out/circle/Dist_s%.2f.mat', sigma);
PyName = sprintf('out/circle/Manapprox_Dist_n1000_s%.2f.mat',sigma);
...
if NumTrials > 1
    ...
    %a.YScale = 'log';
    %ylim([0.001,0.1])
    title(['d = 1, \sigma = ', num2str(sigma)], 'FontSize',16)

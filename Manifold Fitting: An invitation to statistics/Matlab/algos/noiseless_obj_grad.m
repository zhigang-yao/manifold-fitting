function [flag,f, G, H] = noiseless_obj_grad(P, Ui, x, r)
...
for i = 1 : num_idx
    ...
    %Hi(:,:,i) = ((a1+a2)*(dp*dp')+a3)/(2*r);
    sum_Hi = sum_Hi + ((a1+a2)*(dp*dp')+a3)/(2*r);
...
for i = 1 : num_idx
    ...
    a2 = 1/(2*r)/dists(i)*fun_ddtheta(di(i));
    a3 = dists(i)^(-0.5)*fun_dtheta(di(i));
    Hi = ((a1+a2)*(dp*dp')+a3)/(2*r);
    H = H + 2*(a*b'+b*a')-fi(i)/alpha*(da*a'+a*da'-Hi+alpha_i(i)*sum_Hi);
end
H = H + 2*Ax;
%disp(H);

...
import scipy.io
from torusUnif import torusUnif
from pdtorus import pdtorus
...
if n == 500:
    if sigma == 0.09:
        r1 = 1.2*math.sqrt(sigma)
        r2 = 1.4*math.sqrt(sigma) 
        r3 = 1*math.sqrt(sigma)
    elif sigma == 0.08:
        r1 = 1.2*math.sqrt(sigma) 
        r2 = 1.6*math.sqrt(sigma)
        r3 = 1.2*math.sqrt(sigma)
    elif sigma == 0.07:
        r1 = 1.4*math.sqrt(sigma)
        r2 = 1.4*math.sqrt(sigma)
        r3 = 1.2*math.sqrt(sigma)
    elif sigma == 0.06:
        r1 = 1.4*math.sqrt(sigma)
        ...
        r1 = 1.6*math.sqrt(sigma)
        r2 = 1.6*math.sqrt(sigma)
        r3 = 1.2*math.sqrt(sigma)
    elif sigma == 0.02:
        r1 = 1.6*math.sqrt(sigma)
        r2 = 1*math.sqrt(sigma)
        r3 = 1.6*math.sqrt(sigma)
    elif sigma == 0.01:
        r1 = 1*math.sqrt(sigma)
        r2 = 1*math.sqrt(sigma)
        r3 = 1.6*math.sqrt(sigma)
...
for rep in range(NumTrials):
    ...
    fname = 'simulations\\torus\\n%d_s%.2f_trial%d.pkl' % (n, sigma, (rep + 1))
    
    # Loading data from MALTAB
    ...
    try:
        ...
        samples = samples + sigma*np.random.randn(D, NumSample)
        
        data_ini = torusUnif(NumIni, a, b)
        data_ini = data_ini + 0.5*math.sqrt(sigma)/math.sqrt(D)*(2*np.random.rand(D, NumIni) - 1)
    ...
    # fname = 'out/torus/Dist_t%.2f_s%.2f.pkl' % (tau, sigma)
    # with open (fname, 'wb') as f:
...
fname = 'out\\torus\\Dist_n%d_s%.2f.pkl' % (n, sigma)
with open (fname, 'wb') as f:
...
# boxplot
MatName = 'out\\torus\\Dist_n%d_s%.2f.pkl' % (n, sigma)
PyName = 'out\\torus\\Manapprox_Dist_n%d_s%.2f.mat' % (n, sigma)
...
if NumTrials > 1:
    ...
    plt.title('d = ' + str(dim) + ', ' + chr(963) + ' = ' + str(sigma) + ', n = ' + str(n))
    fig.savefig('figures\\torus_max_n%d_s%.2f.png' % (n, sigma))
    
# Check the result differences between Python and MATLAB
# New = 'out\\torus\\Dist_n%d_s%.2f.pkl' % (n, sigma)
# Origin = 'out\\torus\\Dist_s%.2f.mat' % (sigma)
...
# fname = 'out\\torus\\Differences_n%d_s%.2f.pkl' % (n, sigma)
# with open (fname, 'wb') as f:

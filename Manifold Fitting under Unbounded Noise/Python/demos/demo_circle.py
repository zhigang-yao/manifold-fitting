...
for rep in range(NumTrials):
    ...
    fname = 'simulations\\circle\\circle_t%d_n%d_s%.2f_trial%d.pkl' % (tau, Num, sigma, (rep + 1))
    ...
    try:
        ...
        t = np.random.rand(1, NumSample)*2*math.pi
        samples = np.vstack((np.cos(t), np.sin(t))) + sigma*np.random.randn(2, NumSample)
        
        t = np.random.rand(1, NumIni)*2*math.pi
        data_ini = np.vstack((np.cos(t), np.sin(t))) + 0.5*math.sqrt(sigma)/math.sqrt(D)*(2*np.random.rand(2, NumIni)-1)
    ...
    # fname = 'out/circle/Dist_t%d_s%.2f_r%.2f.pkl' % (tau, sigma, r)
    # with open (fname, 'wb') as f:
...
fname = 'out\\circle\\Dist_n%d_s%.2f.pkl' % (Num, sigma)
with open (fname, 'wb') as f:
...
# boxplot
MatName = 'out\\circle\\Dist_n%d_s%.2f.pkl' % (Num, sigma)
PyName = 'out\\circle\\Manapprox_Dist_n%d_s%.2f.mat' % (Num, sigma)
...
if NumTrials > 1:
    ...
    fig.savefig('figures\\circle_max_n%d_s%.2f.png' % (Num, sigma))

# Check the result differences between Python and MATLAB
# New = 'out\\circle\\Dist_n%d_s%.2f.pkl' % (Num, sigma)
# Origin = 'out\\circle\\Dist_n%d_s%.2f.mat' % (Num, sigma)
...
# fname = 'out\\circle\\Differences_n%d_s%.2f.pkl' % (Num, sigma)
# with open (fname, 'wb') as f:
